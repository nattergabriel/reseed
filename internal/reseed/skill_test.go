...
func TestListSkills_NonExistent(t *testing.T) {
	skills, err := listSkills(filepath.Join(t.TempDir(), "nope"))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if skills != nil {
...
func TestFindSkills(t *testing.T) {
	...
	if len(skills) != len(want) {
		t.Fatalf("got %d skills %v, want %d", len(skills), skills, len(want))
	}
	for i, w := range want {
		if skills[i].Name != w.name || skills[i].Group != w.group {
			t.Errorf("skill %d: got {Name: %s, Group: %s}, want {Name: %s, Group: %s}",
				i, skills[i].Name, skills[i].Group, w.name, w.group)
		}
	}
}

func TestFindSkills_NonExistent(t *testing.T) {
	skills, err := FindSkills(filepath.Join(t.TempDir(), "nope"))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if skills != nil {
